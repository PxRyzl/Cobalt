...
from core.automatic_conversion import AutomaticConversion
...
class AutoWindow(QMainWindow):
    ...
    def __init__(self, main):
        ...
        # Cursor Selection
        group_cursor = QGroupBox('Automatic Conversion')
        layout_group_cursor = QGridLayout(group_cursor)
        group_cursor.setLayout=(layout_group_cursor)

        label = QLabel('Select the directory with the cursors')
    ...
    def continue_button_pressed(self):
        ...
        file_path_dir = self.file_path.text()
        AutomaticConversion(file_path_dir, theme_name, theme_com)
